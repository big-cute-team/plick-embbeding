...
- 형식: **마크다운 표**(`| 번호 | 작업 | 설명 |` + 구분선 `|---|---|---|`,
  셀 구분자는 일반 파이프 `|`)로 낸다. 박스 문자(`│`)로 손 정렬하지 않는다 —
  한글은 터미널에서 전각(2칸)이라 손 정렬이 어긋난다. 폭 계산은 렌더러에
  맡기고, 박스 문자와 마크다운을 **섞지 않는다**. 추천 항목 설명 끝에 `⭐ 추천`,
...
| 행동 규칙(골든 루프·브랜치·커밋·Jira·실험 분석) | `docs/AGENT_GUIDE.md` |
