...
- [x] **T05** (KAN-289) 여러 날 이어지는 이슈 대응 — ✔ `scripts/sweep_saga.py` +
  `cluster_incrementally` 수명 연장 옵션 + 위키 `사가_대응_비교.md`. 범위 늘리기·수명 연장 둘 다
  이 데이터 사가엔 무효 — **범위 늘리면 ARI 하락**(0.8636→0.8148/0.8293, 잘못 합침↑), 수명
  연장은 첫 간격부터 넓어 발동 안 함. **24h 유지, 진짜 해법은 주체 연결(KAN-275).**
...
- **P07-T05 완료 · Phase 07 종료(2026-07-23, KAN-289)**: 사가 대응 `scripts/sweep_saga.py`
  ...
  (0.8636→0.8148/0.8293, 옛 같은 이름 다른 사건까지 붙어 잘못 합침↑), 수명 연장은 우리 사가가
  첫 간격부터 넓어(에데르송 ≈48h) 발동 안 함. **24h 유지, 진짜 해법은 시간 아닌 주체 연결
  (KAN-275)**. 수명 연장은 무해해 옵션으로 남김. **Phase 07 done(T01~T05 전부) → Phase 08
  ▶ current.** 순차 묶기 요약: 배치 0.9287 대비 순차 seed 0.8636 → +애매구간 LLM 0.9149(잘못
  합침 0). 브랜치 `feat/KAN-289-saga-window`.
