...
2. 선정된 구성으로 **데이터 수집→임베딩→벡터 저장→증분 dedup 처리**
   파이프라인을 만든다.
3. 검증된 결과는 최종적으로 plick-ai(FastAPI)로 이식한다.

## 핵심 기능

1. 기존 PoC 파이프라인 이관·재현 (임베딩 → 24h 윈도우 → Agglomerative → 리포트)
2. 정답 라벨 + 정량 지표(ARI 등) 평가 러너 — "눈 검증"을 정량화
3. Gemini vs OpenAI 임베딩 모델 벤치마크 (모델 × task_type × 차원 × 임계)
4. 온라인 증분 dedup (발행 시각 순, 24h 활성 이슈와 비교) + 엣지 케이스
   (오병합·다일 사가 분할) 검증, 회색지대 LLM 검수 실험
...
> Obsidian vault(`wiki/`)를 도입했다. 상세는 docs/phases/와 DECISIONS.md.
