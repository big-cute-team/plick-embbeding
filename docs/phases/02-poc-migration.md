...
plick-ai/poc-embedding의 파이프라인(임베딩 → 최근 24시간만 비교 →
...
- [x] P02-T03 묶기 로직 옮겨오기 — 최근 24시간 안의 기사만 비교 대상으로
      좁힌 뒤(최근 시간 범위 비교), 비슷한 것끼리 차례로 합쳐가는 병합형
...
- [x] P02-T04 결과 리포트 만들기 — 어떤 기사끼리 묶였는지와 묶음 분포를
      ...
      텍스트로도 출력한다
- [x] P02-T05 기준선 실험 실행 — 새 데이터셋에 잠정 구성(SEMANTIC@0.85,
      768d, 24h)으로 실험을 돌려 결과를 남기고, 묶음 결과를 눈으로 검증해
      잘 묶인 것/잘못 합침/놓친 것을 리포트에 기록한다. 이 결과가 이후
...
- [x] `scripts/run_experiment.py`가 (모델, task_type, 차원, 임계, 비교 시간 범위)
      파라미터로 실행되어 `results/<타임스탬프>/`에 config+report를 남긴다
- [x] 캐시가 있으면 임베딩 API를 호출하지 않고 같은 결과를 낸다
- [x] 새 데이터셋(`data/articles.json`)이 고정되고, 기준선 실험 1회의
      결과(묶음 수·눈 검증 코멘트)가 리포트에 기록된다
- [x] 파이프라인 단위 테스트(시간 범위 제한·군집화)가 API 없이 통과한다
